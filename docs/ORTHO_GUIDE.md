...
- `ORTHO_REGIME_ROUTER=true` — RANGE→REV / TREND→CONT / EXPANSION→BREAKOUT. 한 번에 이것만 켜고 70/30 검증.
- `ORTHO_TP_REACH_K=1.2` — TP를 `K·ATR·√T_MAX` 도달거리로 상한. 캡처효율↑. 별도 단일검증.
- `ORTHO_P_VOL=70` — BREAKOUT 거래량 서지 컷(R4, 라우터 ON·EXPANSION 전용).
- `ORTHO_CHASE_K=1.0` — CONT 추격 방지(R5, 연장 추세 진입 차단).
- `ORTHO_CORR_DEDUP=true` — 동일 실행 후보를 RR 우선 정렬 후 방향 캡(R6, 상관 클러스터 완화).
- 검증은 신호의 `RG=레짐` 태그로 `ortho_report.py` **Regime·Polarity 코호트**를 보고 +기대값 국면을 확인(R7).

> 과적합 경계: 신규 컷(`VOL_HI`/`P_VOL`/`CHASE_K`/`TP_REACH_K`)은 전부 **백분위·ATR 자기정규화**
> (절대 150%·0.5%·고정 RR 금지), 전부 **기본 OFF**, 변경은 **한 번에 하나** + 70/30. 롱숏 대칭 유지.
