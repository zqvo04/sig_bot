...
## 부록: 진입 가격 기준 — `ORTHO_CLOSED_CANDLES`

- OKX `fetch_ohlcv`는 **형성 중(미완성) 봉을 마지막 원소로** 준다(`confirm=0`).
- **기본 OFF = 실시간 진입(권장):** `entry=c15[-1]`(미완성봉 종가) = 봇 실행 순간의 **시장가**.
  실제 스캘핑처럼 지금 시장가로 들어가는 모델. `Signaled At`=실행 벽시계, resolver는 그 시점부터 채점 → 내부 정합.
  봉 중간값이라 차트의 **완성봉 종가와는 다를 수 있음(정상)** — 종가에 인위적으로 맞추지 않는다.
- **ON(opt-in) = 닫힌봉 앵커링:** 미완성 봉을 드롭 → `entry`·L/F/S 전축·`Signaled At`을 **마지막 닫힌 봉**에
  고정. `Signaled At`=그 봉 종료시각(정확한 경계)이라 **차트에서 1:1 검증 가능**(기록 대조·재현용).
  resolver는 `ts<since`로 진입봉 첫 5m 서브캔들까지 포함.
- 둘 다 **단일변수**(`ORTHO_CLOSED_CANDLES`)로 즉시 전환. 차트 대조 시엔 OKX 차트를 **UTC+9(KST)**로.
