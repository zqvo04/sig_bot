...
- **노브:** `ORTHO_SHADOW_REASONS`(측정 카테고리, 캠페인 집중 시 `MACRO_FRESH`)·
  `ORTHO_SHADOW_MAX_PER_RUN`(런당 write 상한, **0=무제한 기본** — 코인별 OPEN 중복차단이 자연 상한, >0이면 API 보호용 상한)·`NOTION_SHADOW_DB_ID`(다른 DB로 교체 시).
...
## 부록: 닫힌 캔들 앵커링 (기록 무결성) — `ORTHO_CLOSED_CANDLES`

- **문제:** OKX `fetch_ohlcv`는 **형성 중(미완성) 봉을 마지막 원소로** 준다(`confirm=0`). 과거엔 `entry=c15[-1]`가
  봉 중간의 **일시적 스냅샷**이라 차트의 완성봉 종가와 **달랐고**(cron이 봉 경계에 정렬 안 됨), 모든 축도 미완성봉 기준이라 **재현 불가**였다.
- **수정(기본 ON):** 미완성 봉을 드롭 → `entry`·L/F/S 전축·`Signaled At`을 **마지막 닫힌 봉**에 앵커링.
  - `Signaled At` = 그 봉의 **종료시각(=다음 봉 시가시각, 정확한 경계)** → 차트에서 바로 찾아 검증 가능.
  - resolver는 이 시각부터 채점(벽시계 지연 무관). `ts<since` 비교로 **진입봉 첫 5m 서브캔들 포함**.
- **롤백:** `ORTHO_CLOSED_CANDLES=false` → 레거시(미완성봉 스냅샷, `Signaled At`=실행 벽시계). 단일변수.
- **차트 대조 팁:** OKX 차트 시간대를 **UTC+9(KST)**로 두면 `Signaled At`이 진입봉과 1:1 정렬된다.
