...
거부권(MACRO_FRESH·crowd·taker·spread)·추격컷·리스크캡(SLOT/DIRCAP)이 막은 셋업은 그동안
...
- **구축:** `python scripts/create_shadow_db.py <PARENT_PAGE_ID>` (★봇과 동일 `NOTION_TOKEN`으로 실행 →
  인티그레이션 자동 쓰기권한) → 출력된 `database_id`를 Secrets `NOTION_SHADOW_DB_ID`,
  Variables `ORTHO_SHADOW_LOG=true`로 등록. 캠페인 집중 시 `ORTHO_SHADOW_REASONS=MACRO_FRESH`.
- **노브:** `ORTHO_SHADOW_REASONS`(측정 카테고리)·`ORTHO_SHADOW_MAX_PER_RUN`(런당 write 쿼터, 기본 20).
