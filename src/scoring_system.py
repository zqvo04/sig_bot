...
[Fix Issue 2] FVG 양방향 동시 활성 시 보너스 반감 (BONUS_FVG_ENTRY_CONFLICTED)
[Fix Issue 4] 청산 API 패널티 발동 시 캔들 프록시 BONUS_LIQUIDATION 억제
[Fix Issue 7] run_scoring_pipeline에 market_data 파라미터 추가, micro_result 반환
[마이크로구조] calculate_entry_score에 micro_result 파라미터 추가 → 패널티 최종점수 반영
"""
...
def calculate_entry_score(analysis: dict, direction: str,
    ...
    micro_result: compute_microstructure_penalties() 반환값 (optional)
      - total_penalty: 마이크로구조 패널티 합산 (최종점수에 가산)
      - details: [(name, penalty, reason), ...] — 청산 API 발동 여부 판단에 사용
    """
    d    = direction
    ...
    # ③ 대규모 청산  [Fix Issue 4] API 패널티 발동 시 억제
    ...
    # ── FVG 진입 보너스  [Fix Issue 2] 양방향 동시 반감 ──────
    ...
    if both_fvg:
        ...
        logger.info(
            f"[FVG] ⚠️ 양방향 동시 활성 — 보너스 반감 "
            f"({config.BONUS_FVG_ENTRY}pt → {fvg_val}pt) [{d.upper()}]"
        )
    elif d=="long" and bull_fvg:
        bonuses.append(("FVG 강세구간 진입", fvg_val))
        logger.info(f"[FVG] ★ 롱 FVG 진입 +{fvg_val}pt")
    elif d=="short" and bear_fvg:
        bonuses.append(("FVG 약세구간 진입", fvg_val))
        logger.info(f"[FVG] ★ 숏 FVG 진입 +{fvg_val}pt")
    ...
    # ── 최종 점수 ────────────────────────────────────────────
    # soft_penalty = 비율 패널티 (MTF RSI, EXPLOSIVE 소진, 캔들 모멘텀, CHoCH)
    # micro_penalty = 마이크로구조 절대값 패널티 (가산)
    ...
    breakdown=_build_breakdown(d,scores,weights,raw_score,ema_mult,adx_mult,gate_penalty,
                                mtf_penalty,exhaustion_mult,choch_penalty,bonuses,bonus_cap,
                                final_score,gate,regime,micro_penalty)
    return {
...
def _build_breakdown(d, scores, weights, raw, ema_m, adx_m, pen,
                     mtf_m, exh_m, choch_m, bonuses, bonus_cap, final, gate, regime,
                     micro_penalty=0) -> str:
...
def evaluate_signals(analysis: dict,
    ...
    """
    롱/숏 각각 점수 계산 후 주 신호 결정.
    micro_long/micro_short: compute_microstructure_penalties 결과 (optional)
    """
...
def run_scoring_pipeline(symbol: str, analysis: dict,
    ...
    Args:
        symbol:      심볼 문자열 e.g. "BTC/USDT"
        analysis:    analysis_engine.run_full_analysis() 반환값
        market_data: data_pipeline.collect() 반환값 (optional)
                     제공 시 마이크로구조 패널티 계산 및 micro_result 반환에 활용

    Returns:
        pipeline_result dict (notification.notify_signal 입력 형식)
        micro_result 키 포함 (Fix Issue 7)
    """
    ...
    # ── 마이크로구조 패널티 계산 (market_data 제공 시) ──────
    ...
    if market_data:
        try:
            ...
            micro_long = compute_microstructure_penalties(
                ...
                direction="long", regime=regime_name,
                percent_b=percent_b, taker_buy_pct=taker_buy_pct,
                position_long_pct=pos_long_pct,
            )
            micro_short = compute_microstructure_penalties(
                micro_data=micro_data, current_price=price,
                direction="short", regime=regime_name,
                percent_b=percent_b, taker_buy_pct=taker_buy_pct,
                position_long_pct=pos_long_pct,
            )
    ...
    # ── 결과 반환 (Fix Issue 7: micro_result 포함) ────────────
    ...
    return {
        ...
        "micro_result":  micro_result,   # Fix Issue 7
    }
