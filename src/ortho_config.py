...
# ══════════════════════════════════════════════════════════════════
# Shadow 로깅 (FN 측정 인프라 — 별도 Notion DB · ★ 기본 OFF) ──────────
...
# ══════════════════════════════════════════════════════════════════
SHADOW_LOG          = _flag("ORTHO_SHADOW_LOG", "false")
NOTION_SHADOW_DB_ID = os.getenv("NOTION_SHADOW_DB_ID", "")
# 측정 대상 차단 카테고리(쉼표구분). 좁히면 캠페인 집중(예: "MACRO_FRESH"만).
SHADOW_REASONS      = tuple(r.strip().upper() for r in _str(
    "ORTHO_SHADOW_REASONS",
    "MACRO_FRESH,CHASE,CROWD,TAKER,SPREAD,SLOT,DIRCAP").split(",") if r.strip())
SHADOW_MAX_PER_RUN  = _int("ORTHO_SHADOW_MAX_PER_RUN", 20)   # Notion write 쿼터(런당) — API 보호
