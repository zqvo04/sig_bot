...
def analyze_mtf_rsi(df_15m, df_1h, df_4h) -> dict:
    ...
    pullback_long_micro   = (v_1h is not None and v_1h  > 45 and v_15m is not None and v_15m < 45
    ...
    pullback_short_weak   = (v_1h is not None and v_1h  < 48 and v_15m is not None and v_15m > 56
                             and not pullback_short_strong)
    pullback_short_micro  = (v_1h is not None and v_1h  < 55 and v_15m is not None and v_15m > 55
    ...
    div_tag = (
        (" 📊히든롱다이버" if hidden_bull_div else "") +
        (" 📊히든숏다이버" if hidden_bear_div else "")
    )
    logger.info(
        f"[MTF-RSI] 15m:{v15_str} 1h:{v1h_str} 4h:{v4h_str} "
        f"가중:{v_weighted:.1f} [{state}] 롱:{long_score:.1f}pt 숏:{short_score:.1f}pt"
        + pb_tag + div_tag
    )
